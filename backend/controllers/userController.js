...
const forgotPassword = asyncHandler(async (req, res) => {
	...
	//Delete token if it exsists in DB
	...
	await new Token({
		...
		token: hashedToken,
		createdAt: Date.now(),
		expiresAt: Date.now() + 40 * (60 * 1000), // 30 minutes
	}).save()
	...
	try {
		...
		res.status(200).json({ success: true, message: 'Email do zresetowania hasła został wysłany' })
	} catch (error) {
		res.status(500)
...
const resetPassword = asyncHandler(async (req, res) => {
	...
	// Hash token, then compare to Token id DB
	const hashedToken = crypto.createHash('sha256').update(resetToken).digest('hex')
